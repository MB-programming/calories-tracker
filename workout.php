...
<nav class="navbar">
  ...
  <ul class="navbar-nav">
    ...
    <li><a href="workout.php" class="active">🏋️ التمارين</a></li>
...
<div class="container" id="main-content" style="opacity:0">
  ...
  <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem">
    <div>
      <div class="page-title">🏋️ تمارينك</div>
      <div class="page-subtitle">اليوم: <strong style="color:var(--warning)"><?= $todayAr ?></strong></div>
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn btn-primary" onclick="openAddPlanModal()">➕ إضافة تمرين</button>
      <button class="btn btn-secondary" onclick="showView('week')">📅 الخطة الأسبوعية</button>
    </div>
  </div>
  ...
  <div id="view-week" style="display:none">
    <div class="card">
      <div class="card-title" style="justify-content:space-between">
        ...
        <button class="btn btn-secondary btn-sm" onclick="showView('day')">← عودة</button>
      </div>
...
<div class="modal-overlay" id="log-modal">
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title" id="log-modal-title">🏋️ تسجيل التمرين</div>
    ...
    <button class="btn btn-success btn-full" onclick="submitLog()">✅ سجّل التمرين</button>
  </div>
</div>
...
<div class="modal-overlay" id="add-plan-modal">
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title">➕ إضافة تمرين للخطة</div>
    ...
    <button class="btn btn-primary btn-full" onclick="addToPlan()">➕ أضف للخطة</button>
  </div>
</div>
...
<div class="modal-overlay" id="hist-modal">
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title" id="hist-title">📈 تاريخ التمرين</div>
...
let allPlanData   = {};      // keyed by day_of_week
let todayLoggedIds = new Set();
let exercises     = [];

const muscleIcons = {
  'الصدر':'💪','الظهر':'🔙','البايسبس':'💪','التريسبس':'🦾',
  'الأكتاف':'🏋️','الأرجل':'🦵','الساق':'🦵','الكور':'🎯','البطن':'🔥',
};
...
function loadDayPlan(day) {
  ...
  if (!plan.length) {
    wrap.innerHTML = `
      <div style="text-align:center;padding:3rem 1rem;color:var(--text-muted)">
        ...
        <button class="btn btn-primary btn-sm" onclick="openAddPlanModal(${day})">➕ إضافة تمرين</button>
      </div>`;
    updateProgress(day);
    return;
  }
  ...
  wrap.innerHTML = plan.map(item => {
    ...
    const icon    = muscleIcons[item.muscle_group] || '🏋️';
      ...
      <div class="ex-card ${done?'done':''}" id="exc-${item.id}">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end">
          ...
          <button class="btn btn-danger btn-sm btn-icon" style="width:32px;height:32px"
            onclick="removeFromPlan(${item.id})" title="حذف">🗑️</button>
        </div>
      </div>`;
  }).join('');
...
function openLogModal(exerciseId, name, planId, sets, reps, lastWeight) {
  document.getElementById('log-modal-title').textContent = '🏋️ ' + name;
