...
<div class="auth-wrap">
  <div class="auth-card" id="auth-card">
    <div class="auth-logo">
      <span class="icon"><i class="bi bi-<?= $APP_ICON ?>"<?= $APP_COLOR ? ' style="color:'.$APP_COLOR.'"' : '' ?>></i></span>
      <h1><?= $APP_NAME ?></h1>
    ...
    <div id="tab-register" style="display:none">
      <form id="register-form">
        ...
        <button type="submit" class="btn btn-primary btn-full btn-lg" id="register-btn">
          <i class="bi bi-person-plus-fill"></i> إنشاء الحساب
        </button>
      </form>
    </div>
  </div>
...
function showAlert(msg, type='error') {
  document.getElementById('alert-box').innerHTML =
    `<div class="alert alert-${type}"><i class="bi bi-${type==='success'?'check-circle-fill':'x-circle-fill'}"></i> ${msg}</div>`;
}
...
document.getElementById('login-form').addEventListener('submit', async e => {
  ...
  if (data.success) {
    ...
    btn.disabled = false; btn.innerHTML = '<i class="bi bi-box-arrow-in-right"></i> دخول';
  }
});
...
document.getElementById('register-form').addEventListener('submit', async e => {
  ...
  if (data.success) {
    ...
    showAlert(data.message);
    btn.disabled = false; btn.innerHTML = '<i class="bi bi-person-plus-fill"></i> إنشاء الحساب';
  }
});
